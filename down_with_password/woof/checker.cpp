...
bool vis[MAXN][MAXN]; int c[MAXN];

bool a1 = 1, a2 = 1, b = 1;

int main(int argc, char* argv[]) {
	registerTestlibCmd(argc, argv);
	n = inf.readInt(), t = inf.readInt();
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= i; j++) {
			a[i][j] = ouf.readInt();
			if (a[i][j] < 1 || a[i][j] > n) quitf(_wa, "The numbers are not in the valid range.");
		}
	}
	for (int i = 1; i <= n; i++) {
		if (c[a[i][1]]) quitf(_wa, "The first column does not satisfice.");
		c[a[i][1]] = 1;
	}
	for (int i = 2; i <= n; i++) {
		...
		for (int j = 1, x, y; j <= i; j++) {
			...
			if (x > y) swap(x, y);
			if (x == y) a1 = 0;
			if (vis[x][y]) a1 = b = 0; vis[x][y] = 1;
		}
	}
	if (t >= 1 && !a1) quitf(_wa, b ? "The adjacent numbers are not distinct." : "The pairs of numbers are not distinct.");
	if (t >= 2 && !a2) quitf(_wa, "The numbers in a row are not distinct.");
	quitf(_ok, "Well done.");
}
