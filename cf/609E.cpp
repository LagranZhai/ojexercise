...
namespace dsu{
    ...
    int kruskal(){
        std::sort(edge+1,edge+m+1,[](auto A,auto B){return A.w<B.w;});
        int cnt{0},ans{0};
        for(int i{1};i<=m;i++){
            if(cnt==n-1)break;
...
signed main(){
    ...
    dsu::init();
    int mstval=dsu::kruskal();
    dfs1(1,0);
    dfs2(1,0,1);
    build(1,1,n);
    std::sort(edge+1,edge+m+1,[](auto A,auto B){return A.id<B.id;});
    for(int i{1};i<=m;i++){
        if(ismst[i]){
            cout<<mstval<<"\n";
        }
        else{
            cout<<mstval-jump(edge[i].u,edge[i].v)+edge[i].w<<"\n";
        }
    }
    cout.flush();
    return 0;
